...
// front_distance (port 14) - points FRONT wall, used for Y-position correction
// + roller alignment
...
constexpr double HEADING_GATE_DEG =
    25.0; // heading tolerance for wall correction
...
// How long to run the outtake after contact (ms) — gives all balls time to roll
// out
...
//   2. Left sensor applies differential correction — keeps robot parallel to
//   wall
...
// The IMU handles heading. No horizontal tracking (traction wheels don't
// strafe).
lemlib::OdomSensors
    sensors(nullptr, // vertical tracking wheel 1 (motor encoders used instead)
...
//       Tighten these for more precise auton, loosen if robot times out too
//       fast.
//
...
// Lateral (forward/backward) PID - tuned for 200 RPM green gearset, 3.25"
// wheels Increase kP if robot is slow to reach targets. Increase kD if robot
// overshoots.
lemlib::ControllerSettings
    lateral_controller(8,   // kP - start here, tune up/down
                       0,   // kI - leave at 0
...
lemlib::ControllerSettings
    angular_controller(3,   // kP - start here, tune up/down
                       0,   // kI - leave at 0
...
//   deadband  - joystick values below this are treated as 0 (prevents motor
//   whine) minOutput - minimum motor output when joystick exits deadband
//   (prevents stall) curve     - exponential curve factor (higher = more
//   aggressive at full stick)
...
void alignToWall(double target_mm = 20.0, int timeout_ms = 1500) {
  ...
  while (pros::millis() - start < timeout_ms) {
    ...
    if (dist <= 0)
      break; // sensor error, bail
    double error = dist - target_mm;
    if (std::fabs(error) < 5.0)
      break; // within 5mm tolerance
